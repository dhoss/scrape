...
my $yaml;
my $base_url =
"http://yellowpages.com.au/search/postSearchEntry.do?clueType=0&clue=electrical+contractors&locationClue=All+States&x=0&y=0";
...
while ( $mech->follow_link( text_regex => qr/^next$/i ) ) {
    ...
    my $want = scraper {
        process "li.gold", "contractors[]" => scraper {
            ...
            process ".address", address =>
              'TEXT';    # need to split this up into address, state, postcode,
    ...
    $names = $want->scrape( URI->new($base_url) );
    ...
    push @information,
      { contractor => $names, real_website => $site_from_query };
